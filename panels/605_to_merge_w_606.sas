...
* First we aggregate across ordersizes;
proc sort data=combine; by symbol_group type symbol entry date;
proc means data=combine sum noprint; var sharesexecuted; by symbol_group type symbol entry date; output out=tvol sum=tvol;

data combine; merge combine tvol; 
by symbol_group type symbol entry date; 
...
sum(sharesexecuted) = shex
sum(vwppimpact) = vwppimpact;

run; 

* Then type (MKT and MKTBL);
